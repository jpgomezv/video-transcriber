...
static class Program
{
    ...
    static void Main()
    {
        ...
        string script = Path.Combine(baseDir, "gui.py");

        if (!File.Exists(python) || !File.Exists(script))
        {
            MessageBox.Show(
                "No se encontró el entorno de transcripción en:\n" + baseDir +
                "\n\nAbre Transcribir.cmd en esa carpeta para ver el error.",
                "Transcripción de videos", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return;
        }
